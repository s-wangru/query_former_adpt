...
def train_query_former():
    ...
    model.load_state_dict(torch.load('checkpoints/query_former_base.pt'))
    ...
    peft_config = LoraConfig(
        ...
        lora_dropout=0.05,
        target_modules=[
            'linear_q', 'output_layer',
            'layer1'
        ],
        modules_to_save=['prediction_layer', 'layer_norm']
    )
    model = get_peft_model(model, peft_config)

    for name, param in model.named_parameters():
         if not any(module_name in name for module_name in ['lora', 'prediction_layer', 'layer_norm']):
            print(f"freeze parameter: {name}, Shape: {param.shape}")
    ...
    # torch.save(model.state_dict(), 'checkpoints/query_former_base.pt')
    methods = {
